...
//when the user clicks the login button, execute the login function below
...
async function login() {
    ...
    //you want the variables to match your Java LoginDTO class
    let loginDTO = {
        username: username,
        password: password
    }
    //left of the colon is FIELD NAMES. right of the colon is VALUES.
    //Just like in JSON (JavaScript Object Notation)
    ...
    await fetch(url + "auth/login", {
        ...
        //remember, .then() handles successful requersts, .catch() provides error handling, and .finally() always happens
        ...
        .then((data) => {
            ...
            if(parseJwt(data.accessToken).Role === "Financial Manager"){
                //switch to the manager page
                window.location.href = "p1ajmanagers.html"

            } else {
                //send them to the employee page

                // window.location.href = "p1ajemployees.html"

            }


        })
        .catch((error) => document.getElementById("header").innerText = "Login Failed! Try again...")
        // .finally(() => alert("hi"))
